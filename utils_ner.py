    ...
    class NerDataset(Dataset):
        ...
        def __init__(
            ...
            cached_features_file = os.path.join(
                data_dir, "multi_cached_{}_{}_{}".format(
            ...
            with FileLock(lock_path):
                ...
                if os.path.exists(cached_features_file) and not overwrite_cache:
                    ...
                    examples = read_examples_from_file(
                        data_dir, mode, data_list=data_list, eval_data_list=eval_data_list)
...
def read_examples_from_file(data_dir, mode: Union[Split, str], data_list='', eval_data_list='') -> List[InputExample]:
    ...
    def gen_dataset(file_path, guid_index, examples, merge_idx=0, downsample=False, merge_data=""):
        if not os.path.exists(file_path):
            ...
            with open(file_path, encoding="utf-8") as f:
                ...
                for line_idx, line in enumerate(f):
                    if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                        if words:
                            new_ex.append(InputExample(
                                guid=f"{mode}-{guid_index}", words=words, labels=labels, entity_labels=entity_labels))
            ...
            if downsample:
                if merge_data in ["NCBI-disease", "BC4CHEMD", "BC2GM", "linnaeus", "JNLPBA-cl", "JNLPBA-ct", "JNLPBA-dna", "JNLPBA-rna", "BC5CDR-chem"]:
    ...
    if 'train' in mode:
        ...
        for _, merge_data in enumerate(data_list):
            ...
            examples, guid_index = gen_dataset(
                file_path, guid_index, examples, merge_idx=merge_idx, downsample=False)
        ...
        for _, eval_data_type in enumerate(eval_data_list):
            ...
            examples, guid_index = gen_dataset(
                file_path, guid_index, examples, merge_idx=merge_idx)
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm(enumerate(examples)):
        ...
        if pad_on_left:
            ...
            entity_type_ids = ([example.entity_labels[0]]
                               * padding_length) + entity_type_ids
        else:
        ...
        if ex_index < 1:
            ...
            logger.info("entity_type_ids: %s", " ".join(
                [str(x) for x in entity_type_ids]))
