...
def main():
    ...
    wandb.init(project="gerbera", name=data_args.wandb_name)  # need check
    ...
    if "RoBERTa" in model_args.model_name_or_path or "biolm" in model_args.model_name_or_path:
        # roberta
    ...
    # Get datasets-loading and processing data
    train_dataset = (
        NerDataset(
    ...
    if training_args.do_eval:
        ...
        eval_predictions, eval_label_ids, eval_metrics = trainer.predict(eval_dataset)
        ...
        if trainer.is_world_master():
            ...
            with open(output_eval_predictions_file, "w") as writer:
                with open(os.path.join(data_args.data_dir, data_args.eval_data_list, "devel.txt"), "r") as f:  # Assume dev.txt is your eval file
                    example_id = 0
                    for line in f:
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_master():
            with open(output_test_predictions_file, "w") as writer:
                with open(os.path.join(data_args.data_dir, data_args.eval_data_list, "test.txt"), "r") as f:
                    example_id = 0
                    for line in f:
                        if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                            writer.write(line)
                        ...
                        # else:
                        #     logger.warning(
                        #         "Maximum sequence length exceeded: No prediction for '%s'.", line.split()[
                        #             0]
                        #     )
    return results
